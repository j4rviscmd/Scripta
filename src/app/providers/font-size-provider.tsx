 ...
 */
const FontSizeContext = createContext<FontSizeContextState | undefined>(undefined);
...
export function FontSizeProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    configStore.get<number>(FONT_SIZE_STORE_KEY).then((stored) => {
      if (stored != null) {
        setFontSizeState(Math.max(MIN_FONT_SIZE, Math.min(MAX_FONT_SIZE, stored)));
      }
    }).catch((err) => {
