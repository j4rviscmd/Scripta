...
export function useCommandPaletteScroll() {
  ...
  const [enabled, setEnabledState] = useState(DEFAULT_COMMAND_PALETTE_SCROLL.enabled);
  ...
  useEffect(() => {
    Promise.all([
      ...
      .then(([storedEnabled, storedFraction]) => {
        const resolvedEnabled = storedEnabled ?? DEFAULT_COMMAND_PALETTE_SCROLL.enabled;
  ...
  const setEnabled = useCallback(
    (value: boolean) => {
      ...
      configStore.set(COMMAND_PALETTE_SCROLL_STORE_KEYS.enabled, value).catch((err) => {
